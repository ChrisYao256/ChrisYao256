...
if __name__ == '__main__':
	...
	"*** YOUR CODE HERE ***"
	X_space=np.array(np.linspace(0,4,100))
	y_space=np.array(m_opt*X_space+b_opt)
	X_space=np.reshape(X_space,(100,1))
	y_space=np.reshape(y_space,(100,1))
	...
	# W_opt = None
	"*** YOUR CODE HERE ***"
	Id=np.ones_like(X_space)
	X_space_stacked = np.hstack((Id, X_space))
	X_space_T = X_space_stacked.transpose()
	W_opt = np.linalg.solve(np.matmul(X_space_T, X_space_stacked), np.matmul(X_space_T, y_space))
	# print(W_opt)
	...
	# y_pred_rand = []
	"*** YOUR CODE HERE ***"
	y_pred_rand=np.array(m_rand_opt*X_space+b_rand_opt)
	y_pred_rand=np.reshape(y_space,(100,1))
	...
	# It seems like plotting the 100 points with noise just covers most of the graph with points. I commented it out so the actual line is visible. 
